...
addCardBtn.addEventListener("click", newForm = e => {
    addCardBtn.style.position = "absolute";
    addCardBtn.style.opacity = "0";
    addCardBtn.style.top = "-100%";
    htmlToInsert ='<h2 style="font-weight: 400; font-size: 1.5rem">New Item</h2>'+
    ...
    let newCardSubmitForm = document.querySelector(".blank-card #new-card-form");

    newCardSubmitForm.addEventListener("submit", eve => {
        eve.preventDefault();
        let newCard = document.createElement("div")
        newCard.classList.add("card")
        newCardItemName = document.querySelector(".blank-card input#new-item-name").value;
        newCardItemPrice = parseInt(document.querySelector(".blank-card input#new-item-price").value);
        newCard.innerHTML = 
